...
const Index = () => {
  ...
  const confirmDeleteTask = (index) => {
    ...
    setShowConfirmarModal(true); // <== True → true
  };

  const handleDeleteTask = () => {
    if (taskToDelete !== null) {
      const updated = [...tasks]; // Tasks → tasks
  ...
  return (
    <main className="main-container">
      <div className="row g-4">
        {categories.map((cat) => (
          <div
            ...
            key={cat.title}
            onClick={() => setSelectedCategory(cat.title)} // ✅ CORRECTO
      ...
      <div className="mt-5 bg-white shwadow rounded">
        ...
        <ul className="list-group list-groud-flush task-lis-container">
          {filteredTasks.length === 0 ? (
            <li className="p-3 m-1 text-center text-muted alert alert-danger">
              No Task Available.{" "}
            </li>
          ) : (
            filteredTasks.map((task, idx) => (
              <li
                className="list-group-item d-flex justify-content-between align items-start py-2 px-3 my-2 rounded mb-2 shadow-sm task-item"
                key={idx}
              >
                <div>
      ...
      {showConfirmarModal && (
        <>
          <div
            className="position-fixed top-0 start-0 w-100 h-100"
            style={{
              backgroundColor: "rgna(0,0,0,0,5)",
              zIndex: 1050,
            }}
          ...
          >
            ...
            >
              <p className="modal-text my-3">
                {" "}
                Are you sure you want to delente this task?
              </p>
